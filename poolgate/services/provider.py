...
from poolgate.providers.registry import assert_capability
...
class GroqService:
    ...
    def __init__(
        ...
        self._chat_client = ChatCapability()
        self._structured_client = StructuredCapability()
        self._tool_client = ToolCapability()
        self._moderation_client = ModerationCapability()
        self._transcription_client = TranscriptionCapability()
        self._synthesis_client = SynthesisCapability()
    ...
    def _run_rotation_generic(
        ...
        call: Callable[[APIKeyState], _R],
        tokens_extractor: Callable[[_R], tuple[int, int]] | None = None,
        capability: str = "api",
    ) -> _R:
    ...
    async def _async_run_rotation_generic(
        ...
        session_id: str,
        config: RequestConfig,
        call: Callable[[APIKeyState], Awaitable[_R]],
        tokens_extractor: Callable[[_R], tuple[int, int]] | None = None,
        capability: str = "api",
    ) -> _R:
    ...
    def stream(
        ...
        final chunk as chunk.x_groq.usage — see capabilities/chat.py).
        """
        assert_capability(model, "chat")
        rid = str(uuid.uuid4())
        ...
        for attempt in range(attempts):
            ...
            try:
                for delta in self._chat_client.stream(
                    ...
                    api_key_id=key.key_id,
                    request_id=rid,
                    on_usage=lambda u: usage_holder.__setitem__("usage", u),
                ):
