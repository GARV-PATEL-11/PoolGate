"""Centralized logging infrastructure for PoolGate."""
...
class ObservabilityLogger:
    ...
    @contextmanager
    def timed_operation(self, operation: str, ctx: RequestContext | None = None) -> Generator[None, None, None]:
...
class StructuredLogger(ObservabilityLogger):
    ...
    @classmethod
    def from_logger(cls, logger: logging.Logger, debug_mode: bool = False) -> "StructuredLogger":
        obj = object.__new__(cls)
        obj._log = logger
        obj.debug_mode = debug_mode
        return obj
...
class _JsonWriter:
    """Thread-safe append-only JSON-line writer. Silently drops writes when path is None."""
...
class LoggerManager:
    ...
    One instance per ``GroqService``. All file paths sourced exclusively from
    the ``PathConfig`` it receives — no other code creates RotatingFileHandlers.
    """

    def __init__(self, paths: PathConfig, level: str = "INFO", debug: bool = False) -> None:
        self._paths = paths
        self._debug = debug
    ...
    def log_request(self, *, rid: str, sid: str, model: str, capability: str, attempt: int = 1) -> None:
        self._write("request", {"rid": rid, "sid": sid, "model": model, "capability": capability, "attempt": attempt})
    ...
    def log_performance(self, *, rid: str, model: str, latency: float, tokens_in: int, tokens_out: int) -> None:
    ...
    def log_storage(self, *, event: str, tracker: str = "", path: str | Path = "", **extra: Any) -> None:
        self._write("storage", {"event": event, "tracker": tracker, "path": str(path), **extra})
