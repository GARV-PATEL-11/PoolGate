...
class ModelCapabilities(BaseModel):
    ...
    supports_streaming: bool = Field(
        False,
        description="True for chat models exposing stream()/async_stream().",
    )
    ...
    max_tools: int | None = Field(
        default=None,
        ge=0,
        description="Max tool definitions per request, if capped.",
    )
    ...
    available_voices: list[str] | None = Field(
        default=None,
        description="Synthesis-only: valid `voice` values for this model.",
    )
