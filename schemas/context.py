...
class RequestContext(BaseModel):
    ...
    trace_id: str | None = Field(
        default=None,
        description="Distributed tracing correlation ID, e.g. for Langfuse.",
    )
...
class RequestOptions(BaseModel):
    ...
    preferred_api_key_ids: list[str] | None = Field(
        default=None,
        description="Pin this request to specific keys, if possible.",
    )
    ...
    cache: bool = Field(
        True,
        description="Whether PoolGate may serve/store a cached response for this request.",
    )
