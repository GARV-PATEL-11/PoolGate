...
class TranscriptionRequest(BaseModel):
    ...
    audio_filename: str | None = Field(
        default=None,
        description="Original filename, used for MIME-type inference on upload.",
    )
    ...
    prompt: str | None = Field(
        default=None,
        description="Optional vocabulary/spelling priming text.",
    )
    response_format: Literal["text", "json", "verbose_json"] = "text"
    temperature: float = Field(0.0, ge=0.0, le=1.0)
    translate: bool = Field(
        False,
        description="If True, translate to English instead of transcribing in-language.",
    )
