"""Unit tests for poolgate/core/logger.py — StructuredLogger, MetricsCollector, Tracer, mask_key, etc."""
...
from poolgate.core.logger import (
    LoggerManager,
    MetricsCollector,
    ObservabilityLogger,
    RequestContext,
...
class TestLoggerManager:
    ...
    def test_debug_mode_adds_debug_file_handler(self, tmp_path, monkeypatch):
        ...
        debug_handlers = [
            h
            for h in poolgate_logger.handlers
            if isinstance(h, logging.handlers.RotatingFileHandler) and "debug" in str(getattr(h, "baseFilename", ""))
        ]
        assert len(debug_handlers) >= 1


# ---------------------------------------------------------------------------
# Merged from test_observability.py
# ---------------------------------------------------------------------------
