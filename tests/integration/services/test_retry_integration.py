...
class TestIsRateLimit:
    ...
    def test_rate_limit_error_name(self):
        ...
        assert is_rate_limit(RateLimitError("rl")) is True
...
class TestIsAuthError:
    ...
    def test_status_401is_auth_error(self):
        exc = Exception("auth")
        exc.status_code = 401  # type: ignore[attr-defined]
        assert is_auth_error(exc) is True

    def test_status_403is_auth_error(self):
        exc = Exception("forbidden")
        exc.status_code = 403  # type: ignore[attr-defined]
        assert is_auth_error(exc) is True

    def test_authentication_error_typeis_auth_error(self):
        class AuthenticationError(Exception):
            pass

        assert is_auth_error(AuthenticationError("bad creds")) is True
