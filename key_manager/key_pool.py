...
class APIKeyState:
	...
	def health_score(self, max_rpm: int = 30, latency_threshold: float = 3.0) -> float:
		...
		return (10.0 - rpm_penalty - active_penalty - latency_penalty - failure_penalty - recent_failure_penalty -
		        cooldown_penalty)
...
class KeyPool:
	...
	def __init__(self, keys: Iterable[str | APIKeyState] | None = None, *, max_parallel: int = 10) -> None:
	...
	def add_key(self, raw_key: str, *, key_id: str | None = None, max_parallel: int | None = None) -> APIKeyState:
		with self._lock:
	...
	def mark_unhealthy(self, key_id: str, *, failed: bool = False, cooldown_secs: float = 60.0) -> None:
		key = self._require_key(key_id)
		if failed:
			key.mark_failed()
		else:
